...
class MultiResUnet(torch.nn.Module):
    def __init__(self, input_channels, num_classes, alpha=1.67):
        ...
        # Encoder Path
        self.multiresblock1 = Multiresblock(input_channels,32)        #出来是32通道
        self.in_filters1 = int(32*self.alpha*0.167)+int(32*self.alpha*0.333)+int(32*self.alpha* 0.5)    #这里是53
        self.pool1 =  torch.nn.MaxPool2d(2)                            #经过maxpooling，通道数还是不变，32
        self.respath1 = Respath(self.in_filters1,32,respath_length=4)  #输入是53通道？通道数不对啊
        ...
        #EnCODER加入skfusion
        #self.skfusion1 = SKFusion2(256, 512, height=2, reduction=1)
        #self.skfusion2 = SKFusion2(128, 256, height=2, reduction=1)
        #self.skfusion3 = SKFusion2(64, 128, height=2, reduction=1)
        #self.skfusion4 = SKFusion2(32, 64, height=2, reduction=1)
        
        #decoder加入skattention
        #self.skattention1 = SKFusion(51, height=1, reduction=1)
        #self.skattention2 = SKFusion(105, height=1, reduction=1)
        #self.skattention3 = SKFusion(212, height=1, reduction=1)
        #self.skattention4 = SKFusion(426, height=1, reduction=1)
        
        #iAFF和MS-CAM
        self.msCam1 = MS_CAM(channels=51, r=2)
        self.msCam2 = MS_CAM(channels=105, r=2)
        self.msCam3 = MS_CAM(channels=212, r=2)
        self.msCam4 = MS_CAM(channels=426, r=2)
        
        self.iaff1 = iAFF(channels=256, r=2)
        self.iaff2 = iAFF(channels=128, r=2)
        self.iaff3 = iAFF(channels=64, r=2)
        self.iaff4 = iAFF(channels=32, r=1)

        self.conv_final = Conv2d_batchnorm(self.in_filters9, num_classes, kernel_size = (1,1), activation='None')

    def forward(self,x : torch.Tensor)->torch.Tensor:
        x_multires1 = self.multiresblock1(x)                                   #51 channels
        x_multires1 = self.msCam1(x_multires1)
        x_pool1 = self.pool1(x_multires1)
        x_multires1 = self.respath1(x_multires1)
        
        x_multires2 = self.multiresblock2(x_pool1)                                   #105 channels
        x_multires2 = self.msCam2(x_multires2)
        x_pool2 = self.pool2(x_multires2)
        x_multires2 = self.respath2(x_multires2)

        x_multires3 = self.multiresblock3(x_pool2)                                   #212 channels
        x_multires3 = self.msCam3(x_multires3)
        x_pool3 = self.pool3(x_multires3)
        x_multires3 = self.respath3(x_multires3)

        x_multires4 = self.multiresblock4(x_pool3)                                   #426 channels
        x_multires4 = self.msCam4(x_multires4)
        x_pool4 = self.pool4(x_multires4)
        x_multires4 = self.respath4(x_multires4)

        x_multires5 = self.multiresblock5(x_pool4)                                    #853 channels

        #up6 = torch.cat([self.upsample6(x_multires5),x_multires4],axis=1)
        up6 = self.iaff1(self.upsample6(x_multires5),x_multires4)
        x_multires6 = self.multiresblock6(up6)                                    #426 channels

        #up7 = torch.cat([self.upsample7(x_multires6),x_multires3],axis=1)
        up7 = self.iaff2(self.upsample7(x_multires6),x_multires3)
        x_multires7 = self.multiresblock7(up7)                                    #212 channels

        #up8 = torch.cat([self.upsample8(x_multires7),x_multires2],axis=1)
        up8 = self.iaff3(self.upsample8(x_multires7),x_multires2)
        x_multires8 = self.multiresblock8(up8)                                    #105 channels

        #up9 = torch.cat([self.upsample9(x_multires8),x_multires1],axis=1)
        up9 = self.iaff4(self.upsample9(x_multires8),x_multires1)
        x_multires9 = self.multiresblock9(up9)                                    #51 channels
